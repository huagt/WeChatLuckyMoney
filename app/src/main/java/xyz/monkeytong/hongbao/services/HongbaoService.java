...
public class HongbaoService extends AccessibilityService
  ...
  @Override public void onAccessibilityEvent(AccessibilityEvent event) {
    ...
    setCurrentActivityName(event);
        ...
        /* 检测通知消息 */
    if (!mMutex) {
      if (sharedPreferences.getBoolean("pref_watch_notification", false) && watchNotifications(
          event)) {
        return;
      }
  ...
  private boolean watchList(AccessibilityEvent event) {
    ...
    List<AccessibilityNodeInfo> nodes =
        eventSource.findAccessibilityNodeInfosByText(WECHAT_NOTIFICATION_TIP);
    ...
    if (!nodes.isEmpty() && currentActivityName.contains(WECHAT_LUCKMONEY_GENERAL_ACTIVITY)) {
      ...
      if (contentDescription != null && !signature.getContentDescription()
  ...
  private void checkNodeInfo(int eventType) {
    ...
    AccessibilityNodeInfo node1 =
        (sharedPreferences.getBoolean("pref_watch_self", false)) ? this.getTheLastNode(
    ...
    if (node2 != null && "android.widget.Button".equals(node2.getClassName()) && currentActivityName
    ...
    boolean hasNodes =
        this.hasOneOfThoseNodes(WECHAT_BETTER_LUCK_CH, WECHAT_DETAILS_CH, WECHAT_BETTER_LUCK_EN,
  ...
  private void sendComment() {
    try {
      ...
      AccessibilityNodeInfo nodeToInput =
          outNode.getChild(outNode.getChildCount() - 1).getChild(0).getChild(1);
